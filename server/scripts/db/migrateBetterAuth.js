...
export const applyBetterAuthTables = async () => {
  ...
  try {
    ...
    console.log('Better Auth PostgreSQL tables verified/created successfully!');
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('migrateBetterAuth.js')) {
  applyBetterAuthTables().catch(() => {
    process.exitCode = 1;
  });
}
